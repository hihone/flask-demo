...
class RoutingSession(orm.Session):
    ...
    def get_bind(self, mapper=None, clause=None):
        ...
        # 其他请求使用 slave
            ...
            # 如果没有明确指定slave，比如db.session.query这种用法，有可能mapper is None，这时需要明确指定slave
            # if mapper is None:
            #     return state.db.get_engine(self.app, bind=SLAVE_DB)
...
class RouteSQLAlchemy(SQLAlchemy):
    ...
    def get_engine(self, app=None, bind=None):
        ...
        if bind == FIXED_SLAVE_BINDS and 'SQLALCHEMY_BINDS_FIXED_SLAVE_COUNT' in current_app.config and \
            ...
            try:
                return super(RouteSQLAlchemy, self).engine(current_app, fixed_bind)
        ...
        return super(RouteSQLAlchemy, self).engine(app, bind)
